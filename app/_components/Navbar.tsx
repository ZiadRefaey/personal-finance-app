...
export default function Navbar({ session }: { session: any }) {
  ...
  return (
    <>
      ...
      <motion.nav
        className="hidden bg-navbar order-2 xl:order-1 w-full h-full rounded-t-lg xl:rounded-tl-none xl:rounded-r-2xl px-4 pt-2 pb-0 xl:pl-0 xl:pr-6 xl:py-8 2xl:py-10 xl:flex xl:flex-col xl:items-start xl:gap-6"
        layout
      >
        <div className="w-full xl:flex-1 xl:min-h-0">
          <div className="hidden xl:block pl-8 mb-10 2xl:mb-[64px] self-start">
            <Logo isLogoLarge={!isRetracted} />
          </div>
          <ul className="flex items-center justify-between pb-0 h-full p-0 xl:h-auto xl:flex-col xl:justify-start xl:gap-2">
            {NavLinks.map((link) => (
              <NavItem
                key={link.label}
                icon={link.icon}
        ...
        <div className="w-full xl:mt-auto xl:space-y-2">
